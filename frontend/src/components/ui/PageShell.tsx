...
export function PageShell({ eyebrow, title, description, actions, className, children }: PageShellProps) {
  return (
    <div className={clsx("grid gap-6", className)}>
      <section className="relative overflow-hidden rounded-panel border border-white/10 bg-surface-1/86 px-5 py-6 shadow-panel backdrop-blur sm:px-6 sm:py-7">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl space-y-2">
            ...
            {description ? <p className="text-sm leading-6 text-ink-3 sm:text-base">{description}</p> : null}
          </div>
