...
const toggleButtonClassName = (active: boolean) => [
  "rounded-card border px-3 py-2 text-sm font-semibold transition-colors",
  active
...
const choiceButtonClassName = (active: boolean) => [
  "rounded-card border px-3 py-2.5 text-left text-sm transition-colors",
  active
...
function SidebarSection({ title, helper, children, testId }: SidebarSectionProps) {
  return (
    <section className="rounded-card border border-white/8 bg-surface-overlay/55 p-3.5" data-testid={testId}>
      ...
      <div className="flex h-full min-h-0 flex-col">
        <div className="border-b border-white/8 px-4 py-4 sm:px-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="grid grid-cols-2 gap-1 rounded-card border border-white/8 bg-surface-overlay/65 p-1" data-testid="sidebar-mode-switch">
              <button
                type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5" data-testid="sidebar-config-stack">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {PROFILE_OPTIONS.map((option) => (
                    <button
                      ...
                      className={choiceButtonClassName(controls.request.profile === option.value)}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {TICKER_GROUP_OPTIONS.map((option) => (
                    <button
                      ...
                      className={choiceButtonClassName(controls.request.ticker_group === option.value)}
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={choiceButtonClassName(directionSelection === option.value)}
                  >
                    <span className="block font-semibold text-ink-1">{option.label}</span>
                  </button>
                ))}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={choiceButtonClassName(timeframeSelection === option.value)}
                    >
                      <span className="block font-semibold text-ink-1">{option.label}</span>
                      <span className="mt-1 block text-xs text-ink-4">{option.detail}</span>
                    </button>
                  ))}
                </div>

                {controls.mode === "expert" ? (
                  <div className="grid grid-cols-2 gap-2" data-testid="expert-fields">
                    <label className="grid gap-1 text-sm text-ink-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={choiceButtonClassName(strictnessSelection === option.value)}
                      >
                        <span className="block font-semibold text-ink-1">{option.label}</span>
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={choiceButtonClassName(familiaritySelection === option.value)}
                      >
                        <span className="block font-semibold text-ink-1">{option.label}</span>
                      </button>
                    ))}
        ...
        <div className="border-t border-white/8 bg-surface-1/96 px-4 py-4 sm:px-5" data-testid="sidebar-action-zone">
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <p className="eyebrow-label">Run Controls</p>
              <span className={statusChipClassName(workflow.readiness.tone)}>{runScan.isPending ? "Running" : workflow.readiness.title}</span>
            </div>

            <section className="rounded-card border border-white/8 bg-surface-overlay/70 p-3" data-testid="sidebar-plan-summary">
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
              <button
                ...
                disabled={disableRun}
                className="rounded-card border border-accent/25 bg-accent px-4 py-3 text-sm font-semibold text-surface-0 shadow-elevated transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
