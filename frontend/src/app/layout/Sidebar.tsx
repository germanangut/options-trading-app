...
const choiceButtonClassName = (active: boolean) => [
  "min-w-0 rounded-card border px-3 py-2.5 text-center text-sm leading-tight whitespace-normal transition-colors",
  active
...
const choiceGridClassName = "grid grid-cols-2 gap-2";
...
function SidebarSection({ title, helper, children, testId, className }: SidebarSectionProps) {
  return (
    <section className={["rounded-card border border-white/8 bg-surface-overlay/55 p-3.5", className].filter(Boolean).join(" ")} data-testid={testId}>
    ...
    <aside aria-label="Scan command center" className="border-b border-white/8 bg-surface-1/92 lg:border-b-0 lg:border-r" data-testid="scan-sidebar">
      <div className="flex h-full min-h-0 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
          <div className="space-y-3" data-testid="sidebar-config-stack">
            <div className="rounded-panel border border-white/8 bg-surface-1/96 px-4 py-3.5 shadow-panel backdrop-blur" data-testid="sidebar-action-zone">
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                  <button
                    ...
                    disabled={disableRun}
                    className="rounded-card border border-accent/25 bg-accent px-3.5 py-2.5 text-sm font-semibold text-surface-0 shadow-elevated transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
                  ...
                  <button
                    ...
                    className="rounded-card border border-white/10 bg-surface-overlay/70 px-3.5 py-2.5 text-sm font-semibold text-ink-2"
